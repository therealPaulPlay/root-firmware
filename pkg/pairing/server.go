...
func withEncryption(handler func(w http.ResponseWriter, decrypted []byte)) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			...
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Device not paired!"})
			return
		}
		...
		if !ok {
			...
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Camera not initialized!"})
			return
		}
		...
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Failed to derive key!"})
			return
		}
		...
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Failed to create session!"})
			return
		}
		...
		if err != nil {
			w.WriteHeader(http.StatusUnauthorized)
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Failed to decrypt payload!"})
			return
		}
		...
		if err := json.Unmarshal(decrypted, &payload); err != nil {
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Invalid payload format!"})
			return
		}

		if payload.DeviceID != req.DeviceID {
			w.WriteHeader(http.StatusUnauthorized)
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Device ID mismatch!"})
			return
		}
...
func (s *Server) start(port string) error {
	...
	mux.HandleFunc("/pair", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(map[string]any{"success": false, "error": "Invalid public key format!"})
			return
		}
